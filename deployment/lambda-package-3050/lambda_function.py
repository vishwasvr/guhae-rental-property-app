...
def list_properties(event, headers):
    try:
        print(f"Starting list_properties. Table name: {table.name}")
        
        # Try a simple scan first without filters to see if table access works
        response = table.scan(Limit=10)
        print(f"Scan response: {response}")
        
        # Filter for properties in memory for now (simpler debugging)
        all_items = response.get('Items', [])
        properties = []
        for item in all_items:
            if item.get('pk', '').startswith('PROPERTY#'):
                try:
...
def create_property(event, headers):
    # All authenticated users can create properties (they are all owners)
    data = json.loads(event['body'])
    property_id = str(uuid.uuid4())
    
    # Set owner_id - in a real app, extract from JWT token
    ...
    item = {
        ...
        # Store detailed address components for better query capabilities
...
def get_dashboard_stats(event, headers):
    # All authenticated users can view dashboard (they are all owners)
    ...
    # Calculate statistics for property owner
    stats = {
        'total_properties': len(all_properties),
        'active_properties': len([p for p in all_properties if p.get('status') == 'active']),
        'vacant_properties': len([p for p in all_properties if p.get('status') == 'vacant']),
        'total_users': 1,  # Simplified for now
        'total_leases': 0,  # Simplified for now
        'my_properties': len(all_properties),
        ...
        for k, v in item.items():
            if not k.startswith(('pk', 'sk', 'gsi')):
                ...
                if isinstance(v, Decimal):
                    formatted[k] = float(v) if v % 1 else int(v)
                else:
                    formatted[k] = v
        
        # Structure address information if detailed components exist
        if any(key in item for key in ['street_address', 'city', 'county', 'state', 'zip_code']):
            formatted['address'] = {
                'streetAddress': item.get('street_address', ''),
                'street': item.get('street_address', ''),  # API format compatibility
                'city': item.get('city', ''),
                'county': item.get('county', ''),
                'state': item.get('state', ''),
                'zipCode': item.get('zip_code', ''),
                'fullAddress': item.get('address', '')  # Keep the combined address string
            }
            
            # Remove individual address fields from root level
            for addr_field in ['street_address', 'city', 'county', 'state', 'zip_code']:
                formatted.pop(addr_field, None)
        
        formatted.setdefault('images', [])
        return formatted
    except Exception as e:
        print(f"Error formatting property {item}: {str(e)}")
