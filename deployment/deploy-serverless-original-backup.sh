...
package_lambda() {
    ...
    # Copy utils directory for RBAC and other utilities
    cp -r ../../src/utils .
...
upload_static_files() {
    ...
    # Upload all frontend files using sync (best practice)
    echo "   🌐 Syncing all frontend files..."
    aws s3 sync ./src/frontend/ s3://$S3_BUCKET/ \
        --delete \
        --exclude "*.DS_Store" \
        --exclude "*.tmp" \
        --exclude "node_modules/*" \
        --cache-control "text/html:no-cache, text/css:max-age=31536000, application/javascript:max-age=31536000, image/*:max-age=31536000"
