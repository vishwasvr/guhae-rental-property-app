...
def migrate_orphaned_properties(event, headers):
    """Migrate orphaned properties (with owner_id='authenticated-owner') to the current authenticated user."""
    try:
        ...
        print(f"Migrating orphaned properties to owner: {owner_id}")
        
        # Find all properties with the old hardcoded owner_id
        response = table.scan(
            FilterExpression='begins_with(pk, :pk_prefix) AND sk = :sk AND owner_id = :old_owner_id',
            ExpressionAttributeValues={
                ':pk_prefix': 'PROPERTY#',
                ':sk': 'METADATA',
                ':old_owner_id': 'authenticated-owner'
            }
        )
        
        orphaned_properties = response.get('Items', [])
        print(f"Found {len(orphaned_properties)} orphaned properties")
        
        migrated_count = 0
        for property_item in orphaned_properties:
            try:
                # Update the property with the new owner_id
                table.update_item(
                    Key={'pk': property_item['pk'], 'sk': 'METADATA'},
                    UpdateExpression='SET owner_id = :new_owner_id, gsi1pk = :new_gsi1pk, updated_at = :updated_at',
                    ExpressionAttributeValues={
                        ':new_owner_id': owner_id,
                        ':new_gsi1pk': f'OWNER#{owner_id}',
                        ':updated_at': datetime.utcnow().isoformat()
                    }
                )
                migrated_count += 1
                print(f"Migrated property: {property_item['pk']}")
            except Exception as update_error:
                print(f"Error migrating property {property_item['pk']}: {update_error}")
        ...
        return {
            ...
            'body': json.dumps({
                'success': True,
                'message': f'Migrated {migrated_count} orphaned properties to {owner_id}',
                'migrated_count': migrated_count,
                'total_found': len(orphaned_properties)
            })
        }
        
    except Exception as e:
        print(f"Error in property migration: {str(e)}")
        return {
            'statusCode': 500,
            'headers': headers,
...
def list_properties(event, headers):
    try:
        ...
        # TEMPORARY: Auto-migrate orphaned properties for this user
        orphaned_properties = []
        
        for item in all_items:
            if (item.get('pk', '').startswith('PROPERTY#') and 
                item.get('sk') == 'METADATA'):
                if item.get('owner_id') == owner_id:
                    # Property already belongs to this user
                    try:
                        formatted = format_property(item)
                        properties.append(formatted)
                    except Exception as format_error:
                        print(f"Error formatting property {item}: {format_error}")
                elif item.get('owner_id') == 'authenticated-owner':
                    # Orphaned property - auto-migrate it
                    orphaned_properties.append(item)
        
        # Auto-migrate orphaned properties to this user
        if orphaned_properties:
            print(f"Auto-migrating {len(orphaned_properties)} orphaned properties to {owner_id}")
            for property_item in orphaned_properties:
                try:
                    table.update_item(
                        Key={'pk': property_item['pk'], 'sk': 'METADATA'},
                    ...
                    formatted = format_property(property_item)
                    properties.append(formatted)
                    print(f"Auto-migrated property: {property_item['pk']}")
                except Exception as update_error:
                    print(f"Error auto-migrating property {property_item['pk']}: {update_error}")
        
        print(f"Found {len(properties)} properties for owner {owner_id} (including migrated)")
